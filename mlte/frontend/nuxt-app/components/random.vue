<template>
    <div>
      ...
      <div>
        ...
        <br />
        <!-- 3) Language level  -->
        <label><b>Explanatory Resources</b></label>
        <div class="info-container">
          <span class="info-icon">i</span>
          <div class="tooltip">API call for project-specific definition</div>
        </div>
    ...
    <div>
                <UTable :rows="tableData" :columns="columns" >
                    <!-- <template #body-cell="{ item, column, rowIndex}"> -->
                      <template #stak="{ row, column }">
    <UInput v-model="row.stak" placeholder="Enter Stakeholder"/>
  </template>
  
  <template #pur="{ row, column }">
    <UInput v-model="row.pur" placeholder="Enter Purpose"/>
  </template>
  
  <template #conseq="{ row, column }">
    <UInput v-model="row.conseq" placeholder="Enter Consequences"/>
  </template>
